...
function HistoryOfComputing() {
  ...
  const [abacusArray, setAbacusArray] = useState<string | string[]>(toAbacus(0));
    ...
    <>
      ...
      <div className="grid grid-cols-2">
        ...
        <div className="p-24 grid place-content-center">
                ...
                <div key={i+1} className="flex flex-col items-center">
                  {Array.from({ length : 2 }).map((_, index) => (
                    <div
                      ...
                      key={index+1}
                      style={{
                        marginTop: (Array.isArray(abacusArray) ? handleAbacusTwos(i, index) : 0) + (index == 0 ? 24 : 0) + "px",
                ...
                <div key={i+1} className="flex flex-col items-center">
                  {Array.from({ length: 4 }).map((_, index) => (
                    <div
                      key={index + 1}
                      style={{
                        marginTop: 68 + (Array.isArray(abacusArray) ? handleAbacusOnes(i, index) : 0) + index * 24 + "px",
                        transition: "0.2s ease"
                      }}
          ...
          }} className="text-center text-lg mt-4 text-[#8e1537]">{!Array.isArray(abacusArray) ? abacusArray : ""}</p>
        </div>
      </div>
    </>
  );
